...
function negCovidTest() {
    var covidPayHours = $.isNumeric($('#STDPayHours').val()) ? parseFloat($('#STDPayHours').val()) : 0;
    var payDecision = $('#PayDecisionStatus').val();
    if (covidPayHours > 16 & payDecision.includes('Negative')) {
        $('#negCOVIDTest').html("As a reminder, this employee is only eligible for 2 days of COVID Bank Pay. Any additional time hours over 2 days should be added as a PTO adjustment in Pay Section 3.");
    }
    else {
        $('#negCOVIDTest').html("");
    }
...
function calculatePTOWithAdjustments() {
    ...
    totalPTO += $.isNumeric($('#COVIDTestPTOAdjustment').val()) ? parseFloat($('#COVIDTestPTOAdjustment').val()) : 0;
