    ...
    public class LogPayDecisionController : Controller
    {
        ...
        public ActionResult GetLogPayDecision(string id)
        {
            ...
            try
            {
                ...
                hlPD.CovidTestsByEmployee = GetCovidTestModel(empInfo.EmployeeID);
                hlPD.PayPeriodInfo = GetPayPeriodInfo(DateTime.Now);
                DateTime PPStart = (DateTime)hlPD.PayPeriodInfo.StartDate;
                DateTime PayDate = (DateTime)hlPD.PayPeriodInfo.PayDate;
                hlPD.OverallCovidTestStatus = "Not Tested or Awaiting Results";
                foreach (var ct in hlPD.CovidTestsByEmployee){
                    DateTime resulted = (DateTime)ct.Resulted;

                    if (resulted.Date >= PPStart.Date && resulted.Date <= PayDate.Date)
                    {

                        if(ct.Result.Contains("Not Detected") || ct.Result.Contains("Negative"))
                        {
                            hlPD.OverallCovidTestStatus = "Negative";
                        }
                        else if(ct.Result.Contains("Detected") || ct.Result.Contains("Positive"))
                        {
                            hlPD.OverallCovidTestStatus = "Positive";
                            break;
                        }
                        else
                        {
                            hlPD.OverallCovidTestStatus = "Indeterminate";
                        }
                    }
                }
            }
        ...
        #region Get COVID Test List

        [CPTCustomAuthorize(Roles = @"NS\CovidPayHRAppMembers")]
        public List<CovidTestViewModel> GetCovidTestModel(string empID)
        {
            var covidTestDB = db.CovidTests;
            IQueryable<CovidTest> queryTests = covidTestDB;
            queryTests = queryTests.Where(x => x.EmployeeNumber == empID).OrderByDescending(x => x.RESULTED);
            var tests = queryTests.ToList();
            var qTestData = new List<CovidTestViewModel>();
            foreach (var ct in tests)
            {
                qTestData.Add(MapFromModel.MapCovidTestViewModel(ct));
            }
            return qTestData;
        }
        #endregion

        #region Get Pay Period Info

        [CPTCustomAuthorize(Roles = @"NS\CovidPayHRAppMembers")]
        public PayPeriodViewModel GetPayPeriodInfo(DateTime dateTime)
        {
            var date = dateTime.Date;
            var payPeriodDb = db.PayPeriods;
            IQueryable<PayPeriod> queryPPs = payPeriodDb;
            queryPPs = queryPPs.Where(x => x.StartDate <= date && x.PayDate >= date);
            var payPeriod = queryPPs.ToList().First();
            var qPPData = new PayPeriodViewModel();
            qPPData = MapFromModel.MapPayPeriodViewModel(payPeriod);
            return qPPData;
        }
        #endregion
        ...
        [CPTCustomAuthorize(Roles = @"NS\CovidPayHRAppMembers")]
        public PartialViewResult CreateNew(string empID, string fteHour, string rateofPay, string stdElection, string fteStatus, DateTime? lastDayWorked, DateTime? returnToWorkDt, string totalSTDPayHours, string covidTestStatus)
        {
            try
            {
                ...
                TempData["CovidTestStatus"] = covidTestStatus;
        ...
        public ActionResult CreateNew([Bind(Include = "EmployeeID,PayDecisionStatus,ClockedIn_ClearedToWork_Dt,STDPayDtTo,STDPayDtFrom,STDPayHours,PayDecisionDt,STDHoursPaidByNSHGross," +
            ...
            "PTODollarsPaid,PTOHoursPaidWeek1,PTODollarsPaidWeek1,PTOHoursPaidWeek2,PTODollarsPaidWeek2,PTOPayHours,PTOAddedByTimekeeper,TotalPTOAdjustment,COVIDTestPTOAdjustment")] PayDecisionViewModel payDecisionViewModel)
        {
            try
            {
        ...
        public PartialViewResult Edit(int id, string empID, string fteHour, string fteStatus, string rateofPay, string stdElection)
        {
            try
            {
                ...
                PayDecisionViewModel pdVM = new PayDecisionViewModel()
                {
                    ...
                    COVIDTestPTOAdjustment = pD.COVIDTestPTOAdjustment,
                    FTEHours = fteHour,
                    FTEStatus = fteStatus,
                    RateOfPay = rateofPay,
                    STDElection_ThisYear = stdElection,
        ...
        public ActionResult Edit([Bind(Include = "ID,HotlineLogID,EmployeeID,PayDecisionStatus,ClockedIn_ClearedToWork_Dt,STDPayDtTo,STDPayDtFrom,STDPayHours,PayDecisionDt,STDHoursPaidByNSHGross,PTOAdjustment," +
            ...
            "PTOHoursPaidWeek1,PTODollarsPaidWeek1,PTOHoursPaidWeek2,PTODollarsPaidWeek2,PTOPayHours,PTOAddedByTimekeeper,TotalPTOAdjustment, COVIDTestPTOAdjustment")] PayDecisionViewModel payDecisionVM)
        {
            try
            {
                if (ModelState.IsValid)
